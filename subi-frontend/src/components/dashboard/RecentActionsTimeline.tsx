...
interface RecentAction {
  id: string;
  type: 'user_login' | 'user_logout' | 'application_created' | 'application_approved' | 'application_rejected' | 'document_uploaded' | 'document_downloaded' | 'role_assigned' | 'permission_granted' | 'profile_updated';
  userId: string;
  userName: string;
  userAvatar?: string;
  description: string;
...
const actionIcons = {
  user_login: { icon: User, color: 'text-green-600 bg-green-100 dark:bg-green-900/30' },
  user_logout: { icon: User, color: 'text-gray-600 bg-gray-100 dark:bg-gray-900/30' },
  application_created: { icon: FileText, color: 'text-blue-600 bg-blue-100 dark:bg-blue-900/30' },
  application_approved: { icon: CheckCircle, color: 'text-green-600 bg-green-100 dark:bg-green-900/30' },
  application_rejected: { icon: XCircle, color: 'text-red-600 bg-red-100 dark:bg-red-900/30' },
  document_uploaded: { icon: Upload, color: 'text-purple-600 bg-purple-100 dark:bg-purple-900/30' },
  document_downloaded: { icon: Download, color: 'text-indigo-600 bg-indigo-100 dark:bg-indigo-900/30' },
  role_assigned: { icon: Shield, color: 'text-orange-600 bg-orange-100 dark:bg-orange-900/30' },
  permission_granted: { icon: Key, color: 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900/30' },
  profile_updated: { icon: Edit, color: 'text-teal-600 bg-teal-100 dark:bg-teal-900/30' },
};
...
const generateMockActions = (): RecentAction[] => {
  ...
  return mockActions.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
};
