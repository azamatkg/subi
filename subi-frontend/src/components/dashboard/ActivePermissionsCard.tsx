...
export const ActivePermissionsCard: React.FC<ActivePermissionsCardProps> = ({
  ...
  const totalActiveUsers = permissions.reduce(
    (sum, perm) => sum + perm.activeUsers,
    0
  );

  // Group permissions by category for summary
  const categoryStats = permissions.reduce(
    (acc, perm) => {
        ...
        <div className='space-y-4'>
          ...
          {displayedPermissions.map(permission => {
            ...
            const usagePercentage =
              (permission.activeUsers / permission.totalAssigned) * 100;
              ...
              >
                <div className='flex items-start justify-between gap-3'>
                  <div className='flex items-start gap-3 flex-1 min-w-0'>
                    {/* Permission icon */}
                    <div
                      className={cn('p-1.5 rounded-lg', categoryConfig?.color)}
                    >
                      ...
                      <div className='flex items-center gap-3 text-xs'>
                        <div className='flex items-center gap-1'>
                          ...
                          <span className='font-medium'>
                            {permission.activeUsers}
                          </span>
                          <span className='text-muted-foreground'>
                            / {permission.totalAssigned}
                          </span>
                        </div>
            ...
            <div className='flex items-center justify-between text-sm'>
              ...
              <div className='flex items-center gap-2'>
                <Badge variant='outline' className='font-mono'>
                  {displayedPermissions.filter(p => p.activeUsers > 0).length}/
