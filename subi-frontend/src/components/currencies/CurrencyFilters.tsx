...
export const CurrencyFilters: React.FC<CurrencyFiltersProps> = ({
  ...
  const handleFilterChange = <K extends keyof CurrencyFilterState>(
    key: K,
    value: CurrencyFilterState[K]
  ) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleFilterChange('codeFilter', e.target.value.toUpperCase())}
                className="font-mono"
                maxLength={3}
              />
            </div>
              ...
              <div className="flex flex-wrap gap-1">
                {filters.status && (
                  <Badge variant="secondary" className="text-xs">
                    {t('currency.fields.status')}: {t(`references.status.${filters.status.toLowerCase()}`)}
                  </Badge>
                )}
                {filters.codeFilter && (
                  <Badge variant="secondary" className="text-xs">
                    {t('currency.fields.code')}: {filters.codeFilter}
                  </Badge>
                )}
                {filters.searchTerm && (
                  <Badge variant="secondary" className="text-xs">
