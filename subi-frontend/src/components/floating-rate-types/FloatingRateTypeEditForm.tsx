...
import {
  floatingRateTypeEditSchema,
  type FloatingRateTypeEditFormData,
  transformFloatingRateTypeFormData,
  transformFloatingRateTypeApiData,
...
import type { FloatingRateTypeUpdateDto, FloatingRateTypeResponseDto } from '@/types/floatingRateType';

interface FloatingRateTypeEditFormProps {
  floatingRateType: FloatingRateTypeResponseDto;
  onSubmit: (id: number, data: FloatingRateTypeUpdateDto) => Promise<void>;
  onCancel: () => void;
  loading?: boolean;
}

export const FloatingRateTypeEditForm: React.FC<FloatingRateTypeEditFormProps> = ({
  floatingRateType,
  onSubmit,
  onCancel,
  loading = false,
  ...
  const form = useForm<FloatingRateTypeEditFormData>({
    resolver: zodResolver(floatingRateTypeEditSchema),
    defaultValues: transformFloatingRateTypeApiData(floatingRateType),
  });

  const handleSubmit = async (data: FloatingRateTypeEditFormData) => {
    try {
      const transformedData = transformFloatingRateTypeFormData(data);
      // Remove the id from the update data
      const { id: _, ...updateData } = transformedData;
      await onSubmit(data.id, updateData);
    } catch (error) {
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('floatingRateType.editFloatingRateType')}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
              <div>
                ...
                {new Date(floatingRateType.createdAt).toLocaleString()}
              </div>
              <div>
                <span className="font-medium">{t('common.updatedAt')}: </span>
                {new Date(floatingRateType.updatedAt).toLocaleString()}
              </div>
              ...
              {floatingRateType.updatedByUsername && (
                <div>
                  <span className="font-medium">{t('common.updatedBy')}: </span>
                  {floatingRateType.updatedByUsername}
                </div>
              )}
            </div>
