...
export const CreditProgramListPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (filters.termMin !== null) {
      count++;
    }
    if (filters.termMax !== null) {
      count++;
    }
    ...
    if (filters.collateralRequired !== null) {
      count++;
    }
  ...
  const formatAmount = (min?: number, max?: number, currency?: string) => {
    ...
    if (min) {
      return `${t('common.from')} ${min.toLocaleString()} ${currency || ''}`;
    }
    if (max) {
      return `${t('common.to')} ${max.toLocaleString()} ${currency || ''}`;
    }
    return t('common.notSpecified');
  };
  ...
  const formatTerm = (min?: number, max?: number) => {
    ...
    if (min) {
      return `${t('common.from')} ${min} ${t('common.months')}`;
    }
    if (max) {
      return `${t('common.to')} ${max} ${t('common.months')}`;
    }
    return t('common.notSpecified');
  };
        ...
        <div className='space-y-4'>
          ...
          <div className='space-y-3 text-sm'>
            ...
            <div className='flex items-center gap-3'>
              ...
              <div>
                <span className='font-medium'>
                  {t('creditProgram.fields.validPeriod')}:
                </span>
                <span className='ml-2 font-semibold'>
