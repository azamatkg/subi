...
export const CreditProgramDetailPage: React.FC = () => {
  ...
  const canActivate = [ProgramStatusEnum.APPROVED, ProgramStatusEnum.SUSPENDED].includes(program.status);
  const canSuspend = program.status === ProgramStatusEnum.ACTIVE;
  const canClose = [ProgramStatusEnum.ACTIVE, ProgramStatusEnum.SUSPENDED].includes(program.status);
  const canEdit = ![ProgramStatusEnum.CLOSED, ProgramStatusEnum.CANCELLED].includes(program.status);
            ...
            <CardContent className="space-y-4">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Badge variant={program.collateralRequired ? 'default' : 'outline'}>
                    {program.collateralRequired ? t('common.yes') : t('common.no')}
                  </Badge>
                </div>
                  ...
                  <>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {program.acceptedCollateralTypes.map(type => (
                            <Badge key={type} variant="outline">
                              {t(`creditProgram.collateralType.${type.toLowerCase()}`)}
                            </Badge>
                          ))}
                        </div>
                      </div>
                    ...
                    {program.minimumCollateralCoverageRatio && (
                      <div className="space-y-2">
                        ...
                        <p className="font-medium">
                          {formatPercentage(program.minimumCollateralCoverageRatio)}
                        </p>
                      </div>
                    )}
                    ...
                    {(program.collateralValuationRequirementsEn ||
                      ...
                      <div className="space-y-2">
                        <p className="text-sm font-medium text-muted-foreground">
                          {t('creditProgram.fields.collateralValuationRequirements')}
                        </p>
                        <div className="space-y-2">
                          {program.collateralValuationRequirementsEn && (
                            <div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <p className="font-medium text-sm">{program.decisionNameEn}</p>
              ...
              <Button
                ...
                size="sm"
                className="w-full"
                onClick={() => navigate(`${ROUTES.DECISIONS}/${program.decisionId}`)}
              >
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="font-medium text-sm">{program.applicationCount}</p>
              </div>
            </CardContent>
          </Card>
        </div>
